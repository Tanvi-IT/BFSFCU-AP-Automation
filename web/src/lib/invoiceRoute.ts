 ...
 * a "View" the same way.
 *
 * A parked document (credit memo or non-invoice) is not in any invoice queue —
 * it has its own viewer — so it is routed there regardless of status when
 * `documentType` says so.
 */
